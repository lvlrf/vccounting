...
اتصال به Remnawave Panel با استفاده از OpexCore
"""
...
class RemnawaveIntegration(BasePanelIntegration):
    """
    پیاده‌سازی اتصال به Remnawave Panel
    مشابه Marzban اما با API متفاوت
    """
    ...
    def _init_api(self):
        ...
        try:
            from opexcore import RemnawaveAPI
            ...
            logger.info(f"Remnawave API initialized for {self.base_url}")
        except ImportError:
            logger.error("OpexCore not installed")
            raise PanelConnectionError("OpexCore library not found")
        except Exception as e:
            logger.error(f"Failed to initialize Remnawave API: {e}")
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            ...
            logger.info(f"Remnawave connection test successful: {self.base_url}")
            return True
        except Exception as e:
            logger.error(f"Remnawave connection test failed: {e}")
            return False
    ...
    async def create_user(
        ...
        """ایجاد کاربر - پیاده‌سازی مشابه Marzban"""
        try:
            ...
            logger.info(f"Remnawave user created: {username}")
            ...
            logger.error(f"Failed to create Remnawave user {username}: {e}")
            raise PanelOperationError(f"Failed to create user: {e}")

    # بقیه متدها مشابه Marzban است...
    # برای خلاصه‌سازی، فقط امضاهای متدها را می‌نویسیم
